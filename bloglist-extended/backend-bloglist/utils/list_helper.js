...
const favouriteBlog = (blogs) => {
  return blogs.reduce((topVal, currentVal) => {
    return topVal && topVal.likes > currentVal.likes ? topVal : currentVal
  })
}

const mostBlogs = (blogs) => {
  const groupedObj = _.groupBy(blogs, 'author')
  const formattedObj = _.map(groupedObj, (entries, author) => ({
    author,
...
const mostLikes = (blogs) => {
  const groupedObj = _.groupBy(blogs, 'author')
  const formattedObj = _.map(groupedObj, (likes, author) => ({
    author,
    likes: _.reduce(likes, (acca, likes) => acca + likes.likes, 0)
  }))
